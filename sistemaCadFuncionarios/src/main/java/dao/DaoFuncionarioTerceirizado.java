...
public class DaoFuncionarioTerceirizado {
	...
	public boolean salvarNoBanco(FuncionarioTerceirizado funcionarioTerceirizado) {
		...
		String comandoSqlInsert = "INSERT INTO funcionario_terceirizado "
		...
		try {
			...
			preparaComando.setDate(3, java.sql.Date.valueOf(funcionarioTerceirizado.getDataNascimento()));
	...
	public FuncionarioTerceirizado buscarPorCpf(String cpf) {
		...
		try {
			...
			if (resultado.next()) {
				...
				funcionarioTerceirizado.setDataNascimento(resultado.getDate("data_nascimento").toLocalDate());
				funcionarioTerceirizado.setFuncao(Funcao.valueOf(resultado.getString("funcao")));
				funcionarioTerceirizado.setEmpresa(resultado.getString("empresa"));
				funcionarioTerceirizado.setHorasTrabalhadas(resultado.getBigDecimal("horas_trabalhadas"));
	...
	public List<FuncionarioTerceirizado> listarFuncionarioTerceirizado() {
		...
		try {
			...
			while (resultadoDaTabelaDoBanco.next()) {
				...
				funcionarioTerceirizado.setFuncao(Funcao.valueOf(resultadoDaTabelaDoBanco.getString("funcao")));
				funcionarioTerceirizado.setEmpresa(resultadoDaTabelaDoBanco.getString("empresa"));
				funcionarioTerceirizado
						.setHorasTrabalhadas(resultadoDaTabelaDoBanco.getBigDecimal("horas_trabalhadas"));
				funcionarioTerceirizado
						.setDataNascimento(resultadoDaTabelaDoBanco.getDate("data_nascimento").toLocalDate());
	...
	public boolean deletarFuncionarioTerceirizado(String cpf) {
		...
		String comandoSqlDelete = "DELETE FROM funcionario_terceirizado WHERE cpf = ?";
	...
	public boolean atualizarFuncionarioTerceirizado(FuncionarioTerceirizado funcionarioTerceirizado) {
		...
		String comandoSqlUpdate = "UPDATE funcionario_terceirizado "
		...
		try {
			...
			preparaComando.setDate(2, java.sql.Date.valueOf(funcionarioTerceirizado.getDataNascimento()));
			...
			preparaComando.setString(6, Base64.getEncoder().encodeToString("123".getBytes(StandardCharsets.UTF_8)));
